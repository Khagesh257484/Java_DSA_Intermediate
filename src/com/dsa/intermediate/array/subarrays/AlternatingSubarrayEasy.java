...
public class AlternatingSubarrayEasy {
    public static ArrayList<Integer> solve(ArrayList<Integer> A, int B) {
        ...
        for (int start = 0; start < A.size(); start++) {
            if (A.size() - start < len)  // this will run only lest than len & from size we are subtracting value of start one by one
                break;     // 5-0, 5-1, 5-2

            boolean w = true;  // Initially assuming that it is true
            for (int j = start + 1; j < start + len; j++) {
                if (A.get(j) == A.get(j - 1)) {   // If any adjacent is same then returning false and break
                    w = false;
                    break;
                }
            }
            if (w)
                ans.add(start + len / 2);  // start+len/2 : adding mid of indices of len size window as per question
        }
        return ans;
    }

    public static void main(String[] args) {
        ArrayList<Integer> al = new ArrayList<>(Arrays.asList(1, 0, 1, 0, 1));
        ArrayList<Integer> al1 = new ArrayList<>(Arrays.asList(0, 0, 0, 1, 1, 0, 1));
        System.out.println(solve(al,1));
    }
}
